...
## ADR-010 — Deterministic in-process HNSW for the dense channel (measured crossover)

**Decision.** Dense search gains a pure-Rust, deterministic HNSW index (Malkov & Yashunin 2016) cached on the engine, keyed by the exact fingerprint `(embedding_model, COUNT, MAX(chunk id))`. Policy `ann_mode`: `auto` (default) brute-forces below `ann_min_chunks` (20k) and past `max_dense_scan` degrades to HNSW **instead of refusing**; `brute` keeps the v0.9 exact-refuse contract; `hnsw` always uses the index. Graph parameters M=16, efC=200, ef_search=64. **Alternatives.** Persisting the graph beside SQLite (rejected for the prototype: rebuild is cheaper than format-stability risk; revisit if measured rebuild cost dominates); IVF (worse update story at this scale); external ANN crates (violates the zero-dependency contract). **Evidence.** `benchmarks/results/ann-crossover-v0.10.0.txt` (M=16, efC=200, dim 128, clustered): at 100k chunks brute p50 20.24 ms vs HNSW p50 0.30 ms @ recall@10 0.853 (ef=64) and 0.42 ms @ 0.967 (ef=128) — a 48× speedup; break-even ≈ 2.3k dense queries after build; delete-10% rebuild ≈ 1 build (40.4 s vs 46.4 s). At 50k: ef=64 holds recall@10 0.960; ≥20k the default ef=64 holds ≥0.95 through 50k, and ef=128 holds ≥0.99 through 50k. Two scale defects were found and fixed during evaluation: (1) forward `Ord` on the result heap evicted the *best* candidate (recall collapsed to 0.54); (2) ascending-id insertion on cluster-sequential corpora starved cross-cluster bridges — recall *saturated* independent of ef (a standalone 100k probe: 0.937 at ef≥256); deterministic hash-order insertion restored smooth ef-scaling (probe: 0.987 @ ef128, 1.000 @ ef512). Two scale defects were found and fixed during evaluation: (1) forward `Ord` on the result heap evicted the *best* candidate (recall 0.54); (2) ascending-id insertion on cluster-sequential corpora starved cross-cluster bridges — recall *saturated* below 1.0 independent of ef (0.937 at ef≥256 on 100k); deterministic hash-order insertion restored smooth ef-scaling (0.987 @ ef128, 1.000 @ ef512). **Consequences.** Vectors are immutable within a model name (engine invariant), so the fingerprint is exact — inserts/deletes/migrations always move COUNT or MAX(id); re-embedding changes the model name. Invalidation = full rebuild (amortized, quantified). Filtered queries bypass the index (it is unfiltered). Memory ≈ vectors + 8 B/edge (~59 MiB @ 100k × 128-d).
